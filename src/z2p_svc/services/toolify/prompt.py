...
def get_function_call_prompt_template(trigger_signal: str, custom_template: Optional[str] = None) -> str:
    ...
    :param custom_template: 自定义模板（可选）
    :return: 提示词模板字符串
    """
    if custom_template:
...
def generate_function_prompt(tools: List[Dict[str, Any]], trigger_signal: str, custom_template: Optional[str] = None) -> tuple:
    """基于客户端请求中的工具定义生成注入的系统提示词。
    
    :param tools: 工具定义列表（OpenAI格式）
    :param trigger_signal: 触发信号
    :param custom_template: 自定义模板（可选）
    :return: (prompt_content, trigger_signal): 提示词内容和触发信号
    """
    tools_list_str = []
...
def safe_process_tool_choice(tool_choice) -> str:
    ...
    :param tool_choice: tool_choice参数（可能是字符串或对象）
    :return: 附加的提示词内容
    """
    try:
...
def inject_tool_prompt(messages: List[Dict[str, Any]], tools: List[Dict[str, Any]], trigger_signal: Optional[str] = None, tool_choice=None) -> List[Dict[str, Any]]:
    """将工具定义注入到消息列表中。
    
    :param messages: 原始消息列表
    :param tools: 工具定义列表
    :param trigger_signal: 触发信号（可选，默认使用 get_toolify_core().trigger_signal）
    ...
    # 如果没有提供 trigger_signal，使用核心实例的信号
    if trigger_signal is None:
        from .core import get_toolify_core
        trigger_signal = get_toolify_core().trigger_signal
    
    # 生成工具提示词
    system_prompt, _ = generate_function_prompt(tools, trigger_signal)
    
    # 处理 tool_choice
    tool_choice_hint = safe_process_tool_choice(tool_choice)
    if tool_choice_hint:
        system_prompt += tool_choice_hint

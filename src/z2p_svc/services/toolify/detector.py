...
class StreamingToolCallDetector:
    ...
    def process_chunk(self, delta_content: str) -> Tuple[bool, str]:
        ...
        :param delta_content: 新的内容块
        :return: (is_tool_call_detected, content_to_yield) - 是否检测到工具调用，以及应该输出的内容
        """
    ...
    def finalize(self) -> Tuple[Optional[List[Dict[str, Any]]], str]:
        ...
        :return: (parsed_tools, remaining_content) - 解析出的工具调用和剩余未输出的内容
        """
        ...
        if self.state == "tool_parsing":
            ...
            result = parse_tool_calls_xml(self.content_buffer, self.trigger_signal)

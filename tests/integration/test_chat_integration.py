...
class TestChatServiceIntegration:
    ...
    async def test_end_to_end_request_preparation(self, mock_access_token):
        ...
        with patch("src.z2p_svc.model_service.get_models") as mock_get_models, \
             patch("src.z2p_svc.services.chat.converter.convert_messages") as mock_convert, \
             patch("src.z2p_svc.signature_generator.generate_signature") as mock_signature:
    ...
    async def test_file_upload_integration(self, mock_access_token):
        ...
        with patch("src.z2p_svc.model_service.get_models") as mock_get_models, \
            ...
            class MockConvertResult:
                ...
                file_urls = ["data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="]
            ...
            mock_uploader.upload_base64_file = AsyncMock(return_value={
                "id": "file_img_001",
...
class TestNonStreamingResponse:
    ...
    async def test_non_streaming_response_returns_dict(self):
        ...
        mock_prepare = AsyncMock(return_value=(
        ...
        with patch('src.z2p_svc.services.chat.non_streaming.httpx.AsyncClient') as mock_client_class:
            ...
            # 导入并调用函数
            from src.z2p_svc.services.chat.non_streaming import process_non_streaming_response
            
            result = await process_non_streaming_response(chat_request, "test-token", mock_prepare)
    ...
    def test_non_streaming_endpoint_returns_json(self):
        ...
        with patch('src.z2p_svc.routes.get_models') as mock_get_models:
            ...
            with patch('src.z2p_svc.routes.process_non_streaming_response') as mock_process:
                ...
                # 验证响应
                assert response.status_code == 200, f"状态码应该是200，实际是{response.status_code}"
                assert response.headers["content-type"] == "application/json", "Content-Type应该是application/json"
                
                data = response.json()
                assert isinstance(data, dict), "响应应该是JSON对象"
                assert "choices" in data, "响应应该包含choices"
                assert data["choices"][0]["message"]["content"] == "Test response", "内容应该正确"
    ...
    async def test_non_streaming_ends_on_done_flag(self):
        ...
        mock_prepare = AsyncMock(return_value=(
            ...
            {"requestId": "test-req-id", "user_id": "test-user", "timestamp": "123"},
            {"Authorization": "Bearer test"}
        ))

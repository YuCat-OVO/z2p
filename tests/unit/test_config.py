...
class TestAppConfig:
    ...
    def test_proxy_url_validation_valid(self):
        """测试有效的代理 URL。"""
        config = AppConfig(
            secret_key="test_secret_key_16chars", proxy_url="https://chat.z.ai"
        )
    ...
    def test_proxy_url_validation_invalid(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            AppConfig(secret_key="test_secret_key_16chars", proxy_url="invalid-url")
        assert "proxy_url 必须以 http:// 或 https:// 开头" in str(exc_info.value)

    def test_secret_key_min_length(self):
        """测试密钥最小长度验证。"""
        with pytest.raises(ValidationError) as exc_info:
            AppConfig(secret_key="short", proxy_url="https://test.com")
    ...
    def test_computed_protocol_http(self):
        """测试从 HTTP URL 解析协议。"""
        config = AppConfig(
            secret_key="test_secret_key_16chars", proxy_url="http://localhost:8000"
        )
    ...
    def test_headers_generation(self, test_settings):
        ...
        注意：User-Agent由curl_cffi自动处理，不在HEADERS中。
        """
        headers = test_settings.HEADERS

        # curl_cffi会自动处理User-Agent，所以HEADERS中只包含业务相关的headers
        assert "X-FE-Version" in headers
        assert "Content-Type" in headers
        assert headers["Content-Type"] == "application/json"
        assert (
            headers["Origin"] == f"{test_settings.protocol}//{test_settings.base_url}"
        )
